...
public class Part2 {
    ...
    static {
        ...
        CARD_TO_VALUE.put('J', 1);
    }

    public static void main(String[] args) {
        var testInput = Utility.readLines("day07/test_1");
        Utility.check(5905, solve(testInput));
    ...
    private static Type getType(String hand) {
        Map<Integer, Integer> charsCount = "AKQT98765432".chars().boxed().collect(Collectors.toMap(c -> c, c -> 0));
        var jokers = hand.chars().filter(c -> c == 'J').count();
        hand.chars().filter(c -> c != 'J').boxed().forEach(c -> charsCount.put(c, charsCount.get(c) + 1));

        var occurrences = charsCount.values();
        if (occurrences.stream().anyMatch(v -> v == 5 - jokers)) {
            return Type.FIVE_OF_KIND;
        }
        if (occurrences.stream().anyMatch(v -> v == 4 - jokers)) {
            return Type.FOUR_OF_KIND;
        }
        if (occurrences.stream().anyMatch(v -> v == 3 - jokers) && occurrences.stream().filter(v -> v >= 2).count() > 1) {
            return Type.FULL_HOUSE;
        }
        if (occurrences.stream().anyMatch(v -> v == 3 - jokers)) {
            return Type.THREE_OF_KIND;
        }
        if (occurrences.stream().filter(v -> v >= 2).count() > 1) {
            return Type.TWO_PAIR;
        }
        if (occurrences.stream().anyMatch(v -> v == 2 - jokers)) {
